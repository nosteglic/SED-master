...
def parse_args(args):
    ...
    parser.add_argument("--config", default="config/dcase21_bds.yaml", type=str, help="Config file in yaml format")
...
def main(args):
    ...
    model = SEDModel(n_class=len(classes), attention=cfg["model"]["attention"],
                     cnn_kwargs=cfg["model"]["cnn"], rnn_kwargs=cfg["model"]["rnn"])
    ema_model = SEDModel(n_class=len(classes), attention=cfg["model"]["attention"],
                     cnn_kwargs=cfg["model"]["cnn"], rnn_kwargs=cfg["model"]["rnn"])
    ...
    trainer_options = MTBGTrainerOptions(**cfg["trainer_options"])
